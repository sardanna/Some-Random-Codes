...
class maxHeap
{
  private:
      ...
      int size;
  public:
      maxHeap():size(0){}                                                           //Initial size is 0
      int getSize(){                                                                //Returns current size of heap
        return size;
      }
      T getMax(){                                                                   //Returns maximum element of current heap
        return heap[0];
      }
      ...
      void heapifyDown(int index){                                                     //Used when an element is deleted from within
        ...
        largest_element_index = l;
        else
        largest_element_index = index;
        if(r < size and heap[r] > heap[largest_element_index])
        largest_element_index = r;
        if(largest_element_index != index){
          swap(heap[largest_element_index],heap[index]);
          heapifyDown(largest_element_index);
        }
      }
      ...
      T extractMaximum(){                                                               //Removes the current maximum
        T tmp = getMax();
...
int main(){
        maxHeap<char> mh;                                                                      //Can be initialized as follows with any data type as required
        mh.insert('a');
        mh.insert('b');
        mh.insert('c');
        mh.printHeap();
